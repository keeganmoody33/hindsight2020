...
4. **Minimize the derivative.** Track only sanitized facts necessary for the engagement. Quotes, paraphrases, filenames, locators, and hashes must not expose or allow reconstruction of restricted values.
5. **Record provenance safely.** In `sources.md`, use a stable source ID plus a sanitized locator or hash, sensitivity classification, and distribution decision.
